-- Enhanced Transaction History RPC with User and Multi-Account Filtering
-- Fixed version addressing time display, array parameters, and aggregate function nesting

-- Drop existing function if exists
DROP FUNCTION IF EXISTS get_transaction_history(UUID, UUID, DATE, DATE, UUID, TEXT, UUID, UUID, TEXT, UUID, INT, INT);

-- Create enhanced version with TEXT parameter for account IDs (comma-separated)
CREATE OR REPLACE FUNCTION get_transaction_history(
  p_company_id UUID,
  p_store_id UUID DEFAULT NULL,
  p_date_from DATE DEFAULT NULL,
  p_date_to DATE DEFAULT NULL,
  p_account_id UUID DEFAULT NULL,           -- Single account (legacy support)
  p_account_ids TEXT DEFAULT NULL,          -- Multiple accounts as comma-separated string
  p_cash_location_id UUID DEFAULT NULL,
  p_counterparty_id UUID DEFAULT NULL,
  p_journal_type TEXT DEFAULT NULL,
  p_created_by UUID DEFAULT NULL,           -- Filter by created user
...
BEGIN
  -- Parse comma-separated account IDs into array if provided
  ...
  WITH filtered_entries AS (
    SELECT 
      ...
      je.entry_date,
      je.created_at,  -- Use created_at for proper timestamp
      je.description,
      je.journal_type,
      je.is_draft,
      je.created_by,
    ...
    LEFT JOIN journal_lines jl ON je.journal_id = jl.journal_id
    ...
    WHERE 
      ...
      AND (
        -- Check if journal has lines matching account filters
        EXISTS (
          SELECT 1 FROM journal_lines jl2
          WHERE jl2.journal_id = je.journal_id
          AND (
            (p_account_id IS NULL AND v_account_ids_array IS NULL) 
            OR (p_account_id IS NOT NULL AND jl2.account_id = p_account_id)
            OR (v_account_ids_array IS NOT NULL AND jl2.account_id = ANY(v_account_ids_array))
          )
          AND (p_cash_location_id IS NULL OR jl2.cash_location_id = p_cash_location_id)
        )
      )
    ...
    GROUP BY 
      ...
      je.journal_type, je.is_draft, je.created_by, u.first_name, 
      u.last_name, ct.currency_code, ct.symbol
    ORDER BY je.created_at DESC  -- Order by created_at for proper time sorting
    LIMIT p_limit
    OFFSET p_offset
  ),
  entries_with_lines AS (
    SELECT 
      ...
      '[]'::json as attachments -- No attachments table currently
    ...
    GROUP BY 
      ...
      fe.description, fe.journal_type, fe.is_draft, fe.created_by,
      fe.created_by_name, fe.currency_code, fe.currency_symbol,
      fe.total_debit, fe.total_credit, fe.total_amount
  )
  SELECT json_agg(row_to_json(entries_with_lines.*)) INTO v_result
...
-- Drop existing function if exists
DROP FUNCTION IF EXISTS get_transaction_filter_options(UUID, UUID);

-- Enhanced Filter Options RPC with Users
...
BEGIN
  ...
  cash_location_counts AS (
    ...
    GROUP BY cl.cash_location_id, cl.location_name, cl.location_type
  ),
  ...
  SELECT json_build_object(
    ...
    'users', (
      SELECT COALESCE(json_agg(
        json_build_object(
          'id', uc.user_id::text,
          'name', COALESCE(uc.first_name || ' ' || uc.last_name, uc.email),
          'email', uc.email,
...
CREATE INDEX IF NOT EXISTS idx_journal_entries_company_store ON journal_entries(company_id, store_id);
