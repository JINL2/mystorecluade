-- =====================================================
-- DATABASE PERFECT SELECTOR RPC FUNCTIONS
-- Based on exact SUPABASE database structure
-- Drops existing functions first, then creates new ones
-- =====================================================
...
-- Based on exact 'accounts' table structure
-- =====================================================
...
BEGIN
    SELECT json_agg(
        json_build_object(
            ...
            'transactionCount', 0,  -- Static value for now
...
-- Based on exact 'cash_locations' table structure
-- Parameter order: company_id, location_type, store_id
-- =====================================================
...
BEGIN
    ...
    WHERE 
        ...
        AND (
            ...
            OR store_id IS NULL  -- Company-wide locations (store_id IS NULL)
        )
        AND (p_location_type IS NULL OR location_type = p_location_type)
    ORDER BY 
        store_id NULLS FIRST,  -- Company-wide (NULL store_id) first
        location_name;
...
-- Based on exact 'counterparties' table structure
-- =====================================================
...
-- Use these to test the functions after creation
-- =====================================================

-- Test accounts (global - no parameters needed except optional type filter)
-- SELECT get_accounts();
-- SELECT get_accounts('asset');
-- SELECT get_accounts('liability');
-- SELECT get_accounts('income');
-- SELECT get_accounts('expense');
-- SELECT get_accounts('equity');

-- Test cash locations (replace 'your-company-uuid' with actual company_id)
-- SELECT get_cash_locations('your-company-uuid');
-- SELECT get_cash_locations('your-company-uuid', 'bank');
-- SELECT get_cash_locations('your-company-uuid', NULL, 'your-store-uuid');

-- Test counterparties (replace 'your-company-uuid' with actual company_id)
-- SELECT get_counterparties('your-company-uuid');
-- SELECT get_counterparties('your-company-uuid', 'your-store-uuid');
-- SELECT get_counterparties('your-company-uuid', NULL, 'customer');

-- =====================================================
-- FUNCTION SIGNATURES FOR REFERENCE
-- =====================================================
-- get_accounts(account_type)
-- get_cash_locations(company_id, location_type, store_id)  
-- get_counterparties(company_id, store_id, counterparty_type)