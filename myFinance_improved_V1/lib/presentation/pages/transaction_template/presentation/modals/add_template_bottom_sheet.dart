...
import '../components/wizard/step_indicator.dart';
import '../components/wizard/template_basic_info_form.dart';
import '../components/wizard/account_selector_card.dart';
import '../components/wizard/permissions_form.dart';
import '../services/template_form_validator.dart';
...
class _AddTemplateBottomSheetState extends ConsumerState<AddTemplateBottomSheet> {
  ...
  Future<void> _createTemplate() async {
    ...
    try {
      ...
      final result = await TemplateCreationService.createTemplate(
        ref: ref,
      ...
      if (mounted) {
        if (result.isSuccess) {
          await TemplateCreationService.showSuccessDialog(context);
        } else {
          await TemplateCreationService.showErrorDialog(context, result.error!);
        }
      }
    } catch (e) {
      if (mounted) {
        await TemplateCreationService.showErrorDialog(context, e.toString());
      }
    } finally {
      if (mounted) {
