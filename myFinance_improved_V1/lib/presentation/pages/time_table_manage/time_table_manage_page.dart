...
class _ShiftDetailsBottomSheetState extends ConsumerState<_ShiftDetailsBottomSheet> with SingleTickerProviderStateMixin {
  ...
  late num currentBonusAmount; // Add state for current bonus
  ...
  void initState() {
    ...
    // Initialize current bonus amount from card
    final dynamic rawBonusAmount = widget.card['bonus_amount'];
    currentBonusAmount = rawBonusAmount is String 
  ...
  Future<void> _showBonusConfirmationDialog() async {
    // Use the state's current bonus amount
    final num currentBonus = currentBonusAmount;
  ...
  Widget _buildBonusTab(Map<String, dynamic> card) {
    ...
    final num basePay = rawBasePay is String 
        ? num.tryParse(rawBasePay.replaceAll(',', '')) ?? 0 
        : rawBasePay ?? 0;
    // Use the state's current bonus amount instead of card data
    final num bonusAmount = currentBonusAmount;
    final num totalPay = basePay + bonusAmount;
