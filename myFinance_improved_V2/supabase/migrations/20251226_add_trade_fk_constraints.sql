-- Migration: Add Foreign Key Constraints to Trade Tables
-- Date: 2025-12-26
...
--
-- Note: Using SET NULL for optional relationships, CASCADE DELETE for child items
...
-- Migration Complete
-- ============================================================
-- Total FK constraints added: 45
--
-- Main Transaction Tables:
--   - trade_proforma_invoices: 4 FKs (company, store, counterparty, currency)
--   - trade_purchase_orders: 5 FKs (company, store, pi, buyer, currency)
--   - trade_letters_of_credit: 9 FKs (company, store, pi, po, applicant, banks x3, currency)
--   - trade_commercial_invoices: 6 FKs (company, store, lc, shipment, buyer, currency)
--   - trade_shipments: 6 FKs (company, store, lc, po, freight_currency, insurance_currency)
--
-- Item Tables (CASCADE DELETE):
--   - trade_pi_items: 2 FKs (pi, product)
--   - trade_po_items: 3 FKs (po, pi_item, product)
--   - trade_ci_items: 3 FKs (ci, shipment_item, product)
--   - trade_shipment_items: 3 FKs (shipment, po_item, product)
--
-- Supporting Tables:
--   - trade_lc_amendments: 2 FKs (lc, fee_currency)
--   - trade_payments: 6 FKs (company, store, lc, ci, currency, base_currency)
--   - trade_documents: 2 FKs (company, store)
--   - trade_activity_logs: 2 FKs (company, store)
--   - trade_status_history: 2 FKs (company, store)
--   - trade_alerts: 2 FKs (company, store)
-- ============================================================
