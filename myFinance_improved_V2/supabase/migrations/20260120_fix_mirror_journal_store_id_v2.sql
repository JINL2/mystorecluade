-- ============================================================================
-- Migration: Fix Mirror Journal store_id NULL Issue (v2 - Correct Version)
-- Date: 2026-01-20
-- Purpose:
--   1. Fix create_mirror_journal_for_counterparty_utc to auto-derive store_id
--   2. Backfill existing journal_lines with NULL store_id
-- ============================================================================
--
-- Problem:
--   Mirror 저널 생성 시 _linked_company_store_id가 NULL이면
--   journal_entries.store_id와 journal_lines.store_id가 모두 NULL로 생성됨
--   → Balance Sheet에서 store별 필터링 시 이 거래들이 누락됨
--
-- Root Cause:
--   Flutter 앱에서 linkedCounterparty_store_id를 전달하지 않으면
--   create_mirror_journal_for_counterparty_utc에서 _linked_company_store_id가 NULL
--
-- Solution:
--   1. 함수 수정: _linked_company_store_id가 NULL이면
--      p_if_cash_location_id에서 store_id를 자동 조회
--   2. 기존 데이터 백필
-- ============================================================================

-- ============================================================================
-- PART 1: Backfill existing NULL store_id journal_lines
-- ============================================================================

-- 1-1. journal_lines: cash_location_id가 있으면 해당 store_id로 업데이트
UPDATE journal_lines jl
SET store_id = cl.store_id
FROM cash_locations cl
WHERE jl.cash_location_id = cl.cash_location_id
  AND jl.store_id IS NULL
  AND cl.store_id IS NOT NULL;

-- 1-2. journal_entries: journal_lines의 store_id를 참조하여 업데이트
UPDATE journal_entries je
SET store_id = (
  SELECT DISTINCT jl.store_id
  FROM journal_lines jl
  WHERE jl.journal_id = je.journal_id
    AND jl.store_id IS NOT NULL
  LIMIT 1
)
WHERE je.store_id IS NULL
  AND EXISTS (
...
-- ============================================================================
-- PART 2: Fix create_mirror_journal_for_counterparty_utc function
-- ============================================================================

-- Drop existing function first to avoid parameter default conflicts
DROP FUNCTION IF EXISTS create_mirror_journal_for_counterparty_utc(UUID, UUID, JSONB, TIMESTAMPTZ, TEXT, UUID, JSONB, UUID);
...
DECLARE
  ...
  -- 통화 체크용 변수
  _my_currency_id UUID;
  _linked_currency_id UUID;
...
BEGIN
  ...
  -- ============================================================
  -- ★★★ FIX (2026-01-20): Auto-derive store_id from cash_location ★★★
  -- ============================================================
  -- linkedCounterparty_store_id가 NULL이고 p_if_cash_location_id가 있으면
  -- cash_location에서 store_id를 자동으로 조회
  -- ============================================================
  IF _linked_company_store_id IS NULL AND p_if_cash_location_id IS NOT NULL THEN
    SELECT cl.store_id INTO _linked_company_store_id
    FROM cash_locations cl
    WHERE cl.cash_location_id = p_if_cash_location_id;
  END IF;
  -- ============================================================
  -- END FIX
  -- ============================================================
  ...
  -- =====================================================
  -- 통화 체크: base_currency가 다르면 에러 반환
  -- =====================================================
  SELECT base_currency_id INTO _my_currency_id
  FROM companies WHERE company_id = p_company_id;

  SELECT base_currency_id INTO _linked_currency_id
  FROM companies WHERE company_id = _linked_company_id;

  IF _my_currency_id IS DISTINCT FROM _linked_currency_id THEN
    RAISE EXCEPTION 'CURRENCY_MISMATCH: The two companies have different base currencies. The receiving company must process the transaction separately with their own exchange rate.';
  END IF;
  -- =====================================================
  ...
  INSERT INTO debts_receivable (
    ...
    COALESCE(NULLIF(p_debt->>'issue_date', '')::DATE, CURRENT_DATE),
...
'Mirror 저널 생성 함수 - 2026-01-20 store_id 자동 파생 추가:
linkedCounterparty_store_id가 NULL일 때 cash_location에서 store_id를 자동으로 조회하여 사용';
