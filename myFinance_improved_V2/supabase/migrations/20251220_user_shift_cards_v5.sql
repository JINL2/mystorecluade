-- user_shift_cards_v5: problem_details_v2 JSONB 통합
-- 제거: is_late, late_minutes, late_deducut_amount, is_extratime, overtime_minutes,
--       is_valid_checkin_location, checkin_distance_from_store, checkout_distance_from_store,
--       is_reported, is_problem, is_problem_solved
-- 추가: problem_details (problem_details_v2 JSONB)

CREATE OR REPLACE FUNCTION public.user_shift_cards_v5(
  p_request_time timestamp without time zone,
  p_user_id uuid,
  p_company_id uuid,
  p_store_id uuid,
...
/*
============================================
함수: user_shift_cards_v5
설명: 사용자의 월간 시프트 카드 데이터를 조회합니다.
변경: v4 대비 problem_details_v2 JSONB 추가, 개별 문제 컬럼 제거
...
BEGIN
    ...
    IF p_store_id IS NULL THEN
        RAISE EXCEPTION 'store_id cannot be null' USING ERRCODE = 'P0001';
    END IF;

    -- 조회 범위 계산 (해당 월 기준 앞뒤 여유)
    v_start_time := (date_trunc('month', p_request_time) - INTERVAL '2 days') AT TIME ZONE p_timezone;
    v_end_time := (date_trunc('month', p_request_time) + INTERVAL '1 month' + INTERVAL '3 days') AT TIME ZONE p_timezone;
