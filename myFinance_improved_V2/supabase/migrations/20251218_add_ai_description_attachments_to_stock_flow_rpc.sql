-- Migration: Add ai_description and attachments to get_location_stock_flow_v2_utc RPC
-- This allows cash_location feature to show AI descriptions and images like transaction_history

CREATE OR REPLACE FUNCTION get_location_stock_flow_v2_utc(
    p_company_id UUID,
...
BEGIN
    ...
    journal_flows_data AS (
        SELECT
            ...
            -- Add attachments subquery (image URLs only, no OCR)
            COALESCE(
                (
                    SELECT json_agg(
                        json_build_object(
