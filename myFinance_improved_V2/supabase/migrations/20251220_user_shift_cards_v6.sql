-- user_shift_cards_v6: store_id 필터 제거 - 회사 전체 시프트 조회
-- 변경: p_store_id 파라미터를 optional로 만들어 null이면 회사 전체 시프트 반환
-- 이유: UI와 QR 스캔에서 동일한 시프트를 선택하도록 통일

CREATE OR REPLACE FUNCTION public.user_shift_cards_v6(
  p_request_time timestamp without time zone,
  p_user_id uuid,
  p_company_id uuid,
  p_store_id uuid DEFAULT NULL,  -- Optional: null이면 회사 전체 조회
...
/*
============================================
함수: user_shift_cards_v6
설명: 사용자의 월간 시프트 카드 데이터를 조회합니다.
변경: v5 대비 store_id 필터를 optional로 변경 (null이면 회사 전체)
