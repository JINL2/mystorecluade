-- ============================================================================
-- DEPLOYMENT READY: Universal Multi-Currency RPC Function V8 FINAL
-- ============================================================================
-- File: DEPLOY_INSERT_AMOUNT_MULTI_CURRENCY_V8_FINAL_2025-11-23.sql
-- Date: 2025-11-23
-- Version: V8 FINAL (Production Ready)
--
-- FIXES APPLIED IN V8:
--   ✅ Fixed CRITICAL bug: Removed duplicate FOR LOOP in 'bank' CASE
--   ✅ Bank INSERT now correctly uses outer loop variable (v_currency_record)
--   ✅ Prevents bank_amount INSERT when p_entry_type='cash' or 'vault'
--
-- FIXES FROM V7:
--   ✅ Fixed ambiguous column reference: balance_after → cash_amount_entries.balance_after
--   ✅ Fixed vault_amount_line INSERT: line_id → vault_amount_id + all required columns
--   ✅ Fixed bank_amount INSERT: added all required columns (company_id, location_id, etc.)
--   ✅ Verified all table structures match INSERT statements
--
...
BEGIN
  ...
  LOOP
    ...
    CASE p_entry_type
      ...
      WHEN 'vault' THEN
        ...
        LOOP
          ...
          IF v_debit IS NOT NULL OR v_credit IS NOT NULL THEN
            -- ✅ FIX: Use vault_amount_id (not line_id) + all required columns
      ...
      WHEN 'bank' THEN
        -- ✅ V8 FIX: Removed duplicate FOR LOOP
