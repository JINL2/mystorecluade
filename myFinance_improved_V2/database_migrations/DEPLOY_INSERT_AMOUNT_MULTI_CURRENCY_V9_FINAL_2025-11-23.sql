-- ============================================================================
-- DEPLOYMENT READY: Universal Multi-Currency RPC Function V9 FINAL
-- ============================================================================
-- File: DEPLOY_INSERT_AMOUNT_MULTI_CURRENCY_V9_FINAL_2025-11-23.sql
-- Date: 2025-11-23
-- Version: V9 FINAL (Production Ready)
--
-- CRITICAL FIX IN V9:
...
--   ✅ Added net_cash_flow column to track actual change
--
-- FIXES FROM V8:
--   ✅ Removed duplicate FOR LOOP in 'bank' CASE
--   ✅ Bank INSERT now correctly uses outer loop variable
--
-- FIXES FROM V7:
...
--   ✅ Fixed bank_amount INSERT: added all required columns
--
...
BEGIN
  ...
  IF v_method_type = 'stock' THEN
    -- 🔥 V9 FIX: STOCK 방식 - 입력한 현재 잔액이 곧 balance_after
  ...
  INSERT INTO cash_amount_entries (
    ...
    v_balance_before,
    v_balance_after,
    v_net_cash_flow,  -- 🔥 V9: Store net_cash_flow in total_amount column
    v_base_currency_id,
    v_exchange_rates,
