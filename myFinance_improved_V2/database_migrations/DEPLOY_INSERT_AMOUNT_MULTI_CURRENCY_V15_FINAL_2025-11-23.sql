-- ============================================================================
-- DEPLOYMENT READY: Universal Multi-Currency RPC Function V15 FINAL
-- ============================================================================
-- File: DEPLOY_INSERT_AMOUNT_MULTI_CURRENCY_V15_FINAL_2025-11-23.sql
-- Date: 2025-11-23
-- Version: V15 FINAL (Production Ready - Check Constraint Fixed)
--
-- CRITICAL FIXES IN V15:
--   🔥 Fixed check constraint violation for transaction_type
--   ❌ Old: v_transaction_type := 'recount' (violates constraint)
--   ✅ New: v_transaction_type := 'normal' (satisfies constraint)
--
--   🔥 Fixed JSONB array manipulation error in FLOW method (from V14)
--   ❌ Old: v_stock_array := v_stock_array - v_previous_stock_denom;
--   ✅ New: Use jsonb_agg with filtering to rebuild array
--
-- ISSUES FIXED:
--   1. PostgreSQL error: operator does not exist: jsonb - jsonb
--   2. Check constraint violation: cash_amount_entries_transaction_type_check
--      (Only allows: 'normal', 'recount_adjustment', 'opening_balance')
--
...
BEGIN
  ...
  -- ============================================================================
  -- Step 6: Set Transaction Type (✅ V15 FIX: Always use 'normal')
  -- ============================================================================

  -- ✅ Always use 'normal' to satisfy check constraint
  -- Check constraint only allows: 'normal', 'recount_adjustment', 'opening_balance'
  -- RECOUNT operations can be identified by:
  --   - method_type = 'stock'
  --   - vault_amount_line.transaction_type = 'recount'
...
-- 1. Vault IN transaction (should work)
-- 2. Vault OUT transaction (should work)
-- 3. Vault RECOUNT with 0 input (should clear inventory)
-- 4. Multi-currency RECOUNT (VND + USD in one call)
-- ============================================================================
