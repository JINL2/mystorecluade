...
class TemplateCacheRepository {
  // Cache storage
  final Map<String, CacheEntry<List<TransactionTemplate>>> _templatesCache = {};
  final Map<String, CacheEntry<TransactionTemplate>> _templateCache = {};
  ...
  Future<void> cacheTemplates({
    ...
    _templatesCache[cacheKey] = CacheEntry(
      data: List<TransactionTemplate>.from(templates),
  ...
  Future<List<TransactionTemplate>?> getCachedTemplates({
    required String companyId,
    required String storeId,
    bool? isActive,
    String? visibilityLevel,
  ...
  Future<void> cacheTemplate(TransactionTemplate template) async {
    // Ensure cache size limit
    _ensureCacheSize();

    _templateCache[template.templateId] = CacheEntry(
      data: template,
      timestamp: DateTime.now(),
      ttl: _defaultTtl,
    );
  ...
  Future<TransactionTemplate?> getCachedTemplateById(String templateId) async {
  ...
  Future<void> preloadTemplates({
    ...
    final publicTemplates = templates.where((t) => t.visibilityLevel == 'public').toList();
    await cacheTemplates(
      companyId: companyId,
      storeId: storeId,
