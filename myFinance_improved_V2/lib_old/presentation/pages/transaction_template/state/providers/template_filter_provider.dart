...
class TemplateFilter {
  ...
  int get hashCode =>
      ...
      (categoryFilters?.join(',').hashCode ?? 0) ^
...
class TemplateFilterStateNotifier extends StateNotifier<TemplateFilter> {
  ...
  void setCategoryFilters(List<String>? categories) {
    state = state.copyWith(categoryFilters: categories);
  }

  void setVisibilityLevel(String? visibility) {
    state = state.copyWith(visibilityLevel: visibility);
  }
  ...
  void setCashLocationId(String? cashLocationId) {
...
extension TemplateFilterExtension on TemplateFilter {
  ...
  int get totalFiltersCount {
    int count = 0;
    if (accountIds?.isNotEmpty == true) count++;
    if (categoryFilters?.isNotEmpty == true) count++;
    if (counterpartyId?.isNotEmpty == true) count++;
    if (cashLocationId?.isNotEmpty == true) count++;
    if (searchQuery?.isNotEmpty == true) count++;
    if (visibilityLevel != null) count++;
    return count;
  }
}