...
class PaymentScheduleItem {
  ...
  final double amount;
  final String currency;
  final String paymentType; // at_sight, usance_30, usance_60, etc.
  final bool isPaid;
  ...
  bool get isDueSoon => !isPaid && daysUntilDue <= 7 && daysUntilDue >= 0;
}

/// Expiry calendar item for L/C visualization
class ExpiryCalendarItem {
  final String lcId;
  final String lcNumber;
  ...
  /// Days until shipment deadline
  int? get daysUntilShipment =>
      latestShipmentDate?.difference(DateTime.now()).inDays;

  /// Check if expired
  bool get isExpired => DateTime.now().isAfter(expiryDate);

  /// Check if expiring soon (within 7 days)
  bool get isExpiringSoon => !isExpired && daysUntilExpiry <= 7;

  /// Check if shipment is overdue
  bool get isShipmentOverdue =>
      latestShipmentDate != null && DateTime.now().isAfter(latestShipmentDate!);
}
