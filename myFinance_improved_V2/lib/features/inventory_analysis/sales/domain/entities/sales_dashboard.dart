...
class SalesDashboard with _$SalesDashboard {
  ...
  List<String> get warnings {
    ...
    if (growth.revenuePct < 0) {
      result.add('Revenue ${growth.revenuePct.toStringAsFixed(1)}% down');
    }
    if (growth.marginPct < 0) {
      result.add('Margin ${growth.marginPct.toStringAsFixed(1)}% down');
    }
    if (growth.quantityPct < -20) {
      result.add('Sales ${growth.quantityPct.toStringAsFixed(1)}% dropped');
    }
    if (thisMonth.marginRate < 50) {
      result.add('Margin rate ${thisMonth.marginRate.toStringAsFixed(1)}% (below target)');
    }
