...
class _JournalInputPageState extends ConsumerState<JournalInputPage>
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      ref.read(journalEntryStateProvider.notifier).state = JournalEntry(
        entryDate: DateTime.now(),
        transactionLines: const [],
        selectedCompanyId: appState.companyChoosen.isNotEmpty ? appState.companyChoosen : null,
  ...
  Future<void> _addTransactionLine([bool? isDebit]) async {
    ...
    if (result != null) {
      ...
      if (result.counterpartyCashLocationId != null) {
        final currentState = ref.read(journalEntryStateProvider);
        ref.read(journalEntryStateProvider.notifier).state = currentState.copyWith(
          counterpartyCashLocationId: result.counterpartyCashLocationId,
        );
      }
    }
  }
