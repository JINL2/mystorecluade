...
class _SessionCountDetailPageState
  ...
  void _onBack() {
    // Refresh session list before going back
    ref.read(sessionListProvider(widget.sessionType).notifier).refresh();
    Navigator.of(context).maybePop();
  }

  void _onMerge() {
    // Show bottom sheet with available sessions to merge
    showModalBottomSheet<void>(
      context: context,
      backgroundColor: TossColors.white,
      isScrollControlled: true,
      ...
      builder: (context) => _MergeSessionBottomSheet(
...
class _MergeSessionBottomSheet extends ConsumerStatefulWidget {
  final String currentSessionId;
  final String sessionType;
  final void Function(SessionListItem session) onSessionSelected;

  const _MergeSessionBottomSheet({
  ...
  @override
  ConsumerState<_MergeSessionBottomSheet> createState() =>
      _MergeSessionBottomSheetState();
}

class _MergeSessionBottomSheetState
    extends ConsumerState<_MergeSessionBottomSheet> {
  @override
  Widget build(BuildContext context) {
