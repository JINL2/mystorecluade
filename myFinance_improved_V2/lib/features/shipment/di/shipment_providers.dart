...
/// Get shipment by ID usecase provider
final getShipmentByIdUseCaseProvider = Provider<GetShipmentByIdUseCase>((ref) {
  final repository = ref.watch(shipmentRepositoryProvider);
  return GetShipmentByIdUseCase(repository);
});

/// Get shipments by Order ID usecase provider
final getShipmentsByOrderIdUseCaseProvider =
    Provider<GetShipmentsByOrderIdUseCase>((ref) {
  final repository = ref.watch(shipmentRepositoryProvider);
  return GetShipmentsByOrderIdUseCase(repository);
});

/// Create shipment usecase provider
final createShipmentUseCaseProvider = Provider<CreateShipmentUseCase>((ref) {
  final repository = ref.watch(shipmentRepositoryProvider);
  return CreateShipmentUseCase(repository);
});

/// Update shipment usecase provider
final updateShipmentUseCaseProvider = Provider<UpdateShipmentUseCase>((ref) {
  final repository = ref.watch(shipmentRepositoryProvider);
  return UpdateShipmentUseCase(repository);
});

/// Delete shipment usecase provider
final deleteShipmentUseCaseProvider = Provider<DeleteShipmentUseCase>((ref) {
  final repository = ref.watch(shipmentRepositoryProvider);
  return DeleteShipmentUseCase(repository);
});

/// Update shipment status usecase provider
final updateShipmentStatusUseCaseProvider =
    Provider<UpdateShipmentStatusUseCase>((ref) {
  final repository = ref.watch(shipmentRepositoryProvider);
  return UpdateShipmentStatusUseCase(repository);
});

/// Search shipments usecase provider
final searchShipmentsUseCaseProvider = Provider<SearchShipmentsUseCase>((ref) {
  final repository = ref.watch(shipmentRepositoryProvider);
  return SearchShipmentsUseCase(repository);
});

/// Get shipment count by status usecase provider
final getShipmentCountByStatusUseCaseProvider =
    Provider<GetShipmentCountByStatusUseCase>((ref) {
  final repository = ref.watch(shipmentRepositoryProvider);
  return GetShipmentCountByStatusUseCase(repository);
});

/// Get shipment items usecase provider
final getShipmentItemsUseCaseProvider =
    Provider<GetShipmentItemsUseCase>((ref) {
  final repository = ref.watch(shipmentRepositoryProvider);
  return GetShipmentItemsUseCase(repository);
});

/// Add shipment item usecase provider
final addShipmentItemUseCaseProvider = Provider<AddShipmentItemUseCase>((ref) {
  final repository = ref.watch(shipmentRepositoryProvider);
  return AddShipmentItemUseCase(repository);
});

/// Update shipment item usecase provider
final updateShipmentItemUseCaseProvider =
    Provider<UpdateShipmentItemUseCase>((ref) {
  final repository = ref.watch(shipmentRepositoryProvider);
  return UpdateShipmentItemUseCase(repository);
});

/// Delete shipment item usecase provider
final deleteShipmentItemUseCaseProvider =
    Provider<DeleteShipmentItemUseCase>((ref) {
  final repository = ref.watch(shipmentRepositoryProvider);
  return DeleteShipmentItemUseCase(repository);
});
