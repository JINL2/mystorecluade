...
class LuxAppV1SupabaseUser extends BaseAuthUser {
  LuxAppV1SupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> luxAppV1SupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = LuxAppV1SupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
