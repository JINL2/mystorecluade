...
class NavBar {
    constructor(options = {}) {
        ...
        // Track current page for dropdown highlighting
    ...
    /**
     * Detect the current page action based on URL path
     * @returns {string} The detected page action or null
     */
    detectCurrentPageAction() {
        ...
        const pathMappings = {
            ...
            'dashboard': 'Dashboard',
            // Additional fallback patterns for different URL structures
            '/inventory': 'Inventory',
            '/invoice': 'Invoice',
    ...
    init() {
        ...
        // Ensure current page action is detected
        this.currentPageAction = this.detectCurrentPageAction();
        ...
        return dropdown.map(section => `
            <div class="navbar-dropdown-section">
                ...
                ${section.items.map(item => {
                    const isActiveDropdownItem = this.currentPageAction === item.action;
                    const activeClass = isActiveDropdownItem ? ' active' : '';
                    console.log('🎯 Dropdown item:', item.label, 'action:', item.action, 'current:', this.currentPageAction, 'isActive:', isActiveDropdownItem);
                    return `
                        <a href="${item.href}" 
                           class="navbar-dropdown-link${activeClass}" 
    ...
    getAssetPath(filename) {
        ...
        if (typeof window !== 'undefined' && window.pathResolver) {
            // PathResolver should return absolute path from website root
            const assetPath = window.pathResolver.resolveAssetPath(filename);
            console.log(`NavBar: Using pathResolver for asset: ${filename} -> ${assetPath}`);
            return assetPath;
        }
        
        // Fallback: Calculate relative path from current location to website root
        const currentPath = window.location.pathname;
        
        // For localhost, extract the path after the website root
        if (currentPath.includes('/mysite/mystorecluade/website/')) {
            // Find the relative path from current location to website root
            const websiteIndex = currentPath.indexOf('/mysite/mystorecluade/website/');
            const pathAfterWebsite = currentPath.substring(websiteIndex + '/mysite/mystorecluade/website/'.length);
            
            // Count directory levels from website root
            const segments = pathAfterWebsite.split('/').filter(s => s && !s.includes('.'));
            const levelsDeep = segments.length;
            
            // Build relative path to go back to website root
            const relativePath = levelsDeep > 0 ? '../'.repeat(levelsDeep) : './';
            const fullPath = relativePath + 'assets/' + filename;
            
            console.log(`NavBar: Current path: ${currentPath}`);
            console.log(`NavBar: Levels deep: ${levelsDeep}`);
            console.log(`NavBar: Asset path: ${fullPath}`);
            
            return fullPath;
        }
        
        // Default fallback
        console.log(`NavBar: Using default asset path for: ${filename}`);
    ...
    highlightActiveDropdownItem() {
        console.log('🎯 highlightActiveDropdownItem called with currentPageAction:', this.currentPageAction);
        
        if (!this.currentPageAction) {
            console.log('⚠️ No current page action detected, skipping highlight');
            return;
        }
        
        // Remove existing active classes from dropdown items
        ...
        document.querySelectorAll('.navbar-dropdown-link').forEach(link => {
            ...
            if (onclickAttr) {
                const actionMatch = onclickAttr.match(/handleDropdownClick\('([^']+)'\)/);
                if (actionMatch && actionMatch[1] === this.currentPageAction) {
                    link.classList.add('active');
                    highlighted = true;
                    console.log('✅ Highlighted dropdown item:', actionMatch[1]);
                    
                    // Also ensure the parent nav item is active
                    const navItem = link.closest('.navbar-nav-item');
                    if (navItem) {
                        const navLink = navItem.querySelector('.navbar-nav-link');
                        if (navLink && !navLink.classList.contains('active')) {
                            // Remove active from all nav links first
                            document.querySelectorAll('.navbar-nav-link').forEach(nl => nl.classList.remove('active'));
                            // Add active to the parent nav link
                            navLink.classList.add('active');
                            console.log('✅ Also highlighted parent nav item:', navLink.textContent.trim());
                        }
                    }
                }
            }
        ...
        if (!highlighted) {
            console.log('⚠️ No dropdown item found matching action:', this.currentPageAction);
            ...
            document.querySelectorAll('.navbar-dropdown-link').forEach(link => {
                ...
                if (onclickAttr) {
                    const actionMatch = onclickAttr.match(/handleDropdownClick\('([^']+)'\)/);
                    if (actionMatch) {
                        availableActions.push(actionMatch[1]);
                    }
                }
            });
            console.log('📋 Available dropdown actions:', availableActions);
        }
    }
    ...
    setActiveItem(itemId) {
        ...
        // Re-detect current page action and highlight dropdown item
        this.currentPageAction = this.detectCurrentPageAction();
        this.highlightActiveDropdownItem();
    ...
    /**
     * Refresh the navbar highlighting - can be called externally
     */
    refreshHighlighting() {
        console.log('🔄 Refreshing navbar highlighting...');
        this.currentPageAction = this.detectCurrentPageAction();
        this.highlightActiveDropdownItem();
...
// Global function to refresh navbar highlighting (can be called from any page)
window.refreshNavbarHighlighting = function() {
    console.log('🔄 refreshNavbarHighlighting called');
    if (window.navbarInstance && window.navbarInstance.refreshHighlighting) {
        const action = window.navbarInstance.refreshHighlighting();
        console.log('✅ Navbar highlighting refreshed, current action:', action);
        return action;
    } else {
        console.log('❌ Navbar instance not found');
        return null;
    }
};